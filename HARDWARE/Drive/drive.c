...
{ 
    ...
    Digital_tube_Infrared_Send(Infrared, 6);
    delay_ms(500);
    Infrared[1] = 0x10;			// 模式
...
u16 Tracking_8;
u16 Tracking_7;
u16 Tracking_value = 0;
uint8_t crossroad;//十字路口检测到白卡
void Car_Track(uint8_t speed)   // 主车循迹 参数：速度
{
    ...
    while(Stop_Flag != 0x01)       // 等待循迹完成
		{
			Tracking_8=Get_Host_UpTrack(TRACK_H8);
			Tracking_7=Get_Host_UpTrack(TRACK_Q7);								
			
			if(Tracking_8 == 0x00)
			{
						Stop_Flag = 1;		//停止循迹	
						Send_UpMotor(0,0);//主车停止			
						break;
			}
			if(terrain == 1)
			{	
...
#if 0				
				if(Tracking_8 == 0xff || Tracking_7 == 0X7F)
				{
					delay_ms(50);
					if(Tracking_8 == 0xff || Tracking_7 == 0X7F)
					{
...
void Car_L45(int8_t speed, uint16_t times)		// 左旋转 参数：旋转时间
{
	...
	for(uint8_t i=0;i<times/500;i++)
	{
		delay_ms(times);					// 延时作角度	
	}
	delay_ms(times%500);					// 延时作角度	
	Send_UpMotor(0 ,0);
	delay_ms(100);
}
...
void Car_R45(int8_t speed, uint16_t tims)		// 右旋转 参数：旋转时间
{
	...
	for(uint8_t i=0;i<tims/500;i++)
	{
		delay_ms(tims);					// 延时作角度	
	}
	delay_ms(tims%500);					// 延时作角度	
