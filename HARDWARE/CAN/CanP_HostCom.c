...
/**
 * @description: 上传字符串调试信息自动换行
 * @param {char} *s 调试信息指针
 * @return {*}
 */
void Send_Debug_string2(char *s)
{	  
	Send_Debug_string(s);
	Send_InfoData_To_Fifo("\n",2);
}
 ...
 * @param {int} num 底数
 * @param {int} n 指数
 * @return {unsigned int} 计算结果
 */
unsigned int power(int num,int n)
{
	int i=1;
	while(n--)
	{
...
/**
 * @description: 上传数字调试信息
 * @param {int} num 上传整数值 超过8位后无效
 * @return {*}
 */
void Send_Debug_num(uint32_t num)
{
	...
	//将整数每个位的值分离出来
	for(i=len,n=0;i>=1;i--,n++)
	{
		Send_Data[n]=num%power(10,i)/power(10,i-1)+48;
	}
	CanDrv_TxData(Send_Data,len,CAN_SID_HL(ID_DISP,0),0,_NULL);
	delay_us(400);
}

/**
 * @description: 上传数字调试信息自动换行
 * @param {int} num 上传整数值 超过8位后无效
 * @return {*}
 */
void Send_Debug_num2(uint32_t num)
{
	Send_Debug_num(num);
	Send_Debug_string("\n");
 ...
 * 返 回 值 ：  无
 * 全局变量 ：  无
 * 备    注 ：  信息大小不得大于 0x98 9680 或 999 9999 
*****************************************************************/
void Send_Debug(uint32_t Debug) 
{	  
	uint8_t Send_Data[11];
	CanDrv_WhaitTxEmpty();
	Send_Data[0]=(Debug%10000000/1000000)+0x30;
	Send_Data[1]=(Debug%1000000/100000)+0x30;
	Send_Data[2]=(Debug%100000/10000)+0x30;
	Send_Data[3]=(Debug%10000/1000)+0x30;
	Send_Data[4]=(Debug%1000/100)+0x30;
	Send_Data[5]=(Debug%100/10)+0x30;
	Send_Data[6]=(Debug%10)+0x30;
	Send_Data[7]='\n';

	CanDrv_TxData(Send_Data,8,CAN_SID_HL(ID_DISP,0),0,_NULL);	
}
 ...
 * 备    注 ：  信息大小不得大于 0x98 9680 或 999 9999 
*****************************************************************/
void Send_Debug_HEX(uint32_t Debug) 
{	  
	uint8_t Send_Data[5];
	CanDrv_WhaitTxEmpty();
	for (int i = 0; i < 25; i++)
	...
	Send_Data[1]='x';
	Send_Data[4]=' ';
	CanDrv_TxData(Send_Data,5,CAN_SID_HL(ID_DISP,0),0,_NULL);	
	delay_us(400);
}
...
返 回 值：Rt 循迹数据
*/
uint16_t  Get_Host_UpTrack( u8 mode)  // 获取循迹数据
{
	uint16_t Rt = 0;
	switch(mode)
	{
...
返 回 值：Rt 循迹数据
*/
uint16_t  Get_Host_TrackDieCount( u8 mode)
{
	uint16_t Rt = 0;
