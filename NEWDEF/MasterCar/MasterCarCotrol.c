...
#define MasterCar_LeftMPV_45 410	   // xc407	   //410 //码盘左转45度
#define MasterCar_RightMPV_45 450	   // xc460	   //450 //码盘右转45度
...
void task_first(void)
{
	...
	MasterCar_Left(MasterCar_TrunSpeed, 1);
	n = OFlag_light2(1); // 路灯调节到1档并获取初始档位
	OFlag_SlaveSendZigbee( 0x07,  n,  n,  n);
	delay_ms(300);
	OFlag_SlaveSendZigbee( 0x07,  n,  n,  n);
	delay_ms(300);
	OFlag_SlaveSendZigbee( 0x07,  n,  n,  n);
...
void task_wait(void)
{
	...
	MasterCar_SmartRunMP(MasterCar_GoSpeed, 800);
...
void task_ETC(void)
{
	...
	OFlag_YYupload(BKRC_Voice_Extern(0, 3));// 小创语音识别3次
	...
	OFlag_SlaveRun();		 // 启动副车
	OFlag_SlaveRun_wait(90); // 等待副车 50s超时
...
void task_RFID(void)
{
...
#if 0
	carRead = RC_Card_checkRangeReadPlus2(0, 2250 , 9, K_A, 1, 0); // 在指定距离内读卡循迹灯版
	
	RC_Card_checkRangeReadPlus2(0, 2250 , 9, K_A, 2-carRead, carRead); // 在指定距离内读卡循迹灯版
#else
	MasterCar_SmartRunMP(MasterCar_GoSpeed,2250*2);
#endif
	...
	OFlag_LED_jl( jl );//LED显示距离模式

	OFlag_SlaveSendZigbee( 0xA0,  jl>>8,  0xff & jl,  0);
	delay_ms(300);
	OFlag_SlaveSendZigbee( 0xA0,  jl>>8,  0xff & jl,  0);
	delay_ms(300);
	OFlag_SlaveSendZigbee( 0xA0,  jl>>8,  0xff & jl,  0);
