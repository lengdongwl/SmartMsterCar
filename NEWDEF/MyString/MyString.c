...
/**
 * @description: 字符串中获取路径
 * 例如：B4D4D6B6|D1，第1个B4开头，以最后1个B6结束，B车初始位置有效信息为D1。
 * get_path_one("B4D4D6B6|D1","B4","B6");
 * @param {char} *str 
 * @param {char} *first 首坐标
 * @param {char} *finally 尾坐标
 * @param {char} *buf 提供缓存数组，缓存所有路径
 * @return {*}
 */
char* get_path_one(char *str,char *first,char *finally,char *buf)
{
	...
	if(str[0]!=first[0] || str[1]!=first[1]) //首位 first
	{
		return 0;
	}
	...
	if(flag)
	{
		if((str[i-2]==finally[0] && str[i-1]==finally[1]) && (len-i-1)==2)//分隔符'|'前面为finally 后面为长度2的坐标
		{
			r[0]=str[i+1];
			r[1]=str[i+2];
			for(j=0;j<i;j++)
