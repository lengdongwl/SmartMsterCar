...
float PID_Track2(unsigned int speed)
{
	...
	case 0xFC: //双灯偏右3
		...
		break;
	default: //0x00&0xff
		if(Get_Host_TrackDieCount(TRACK_H8)>5) //灭5个以上
		{
			error = 99;
			return 99;
		}
...
float PID_TrackMP(unsigned int speed)
{
	...
	case 0xF8://三灯灯偏右3 11111000
		error = 3.2;
		break;
	case 0x1F://三灯灯偏左3 00011111
		error = -3.2;
		break;
...
float PID_Track5(unsigned int speed)
{
	...
	case 0x7E: // 0111 1110 白卡横放
		error = 99;
		break;	
	case 0x3E: // 0011 1110 白卡横放
		error = 99;
		break;	
	case 0x7c: // 0111 1100 白卡横放
		error = 99;
		break;	
	case 0xEF: //11101111 车头在中偏左
		error = -1.2;
...
/*
//python计算error
L = ['1111', '1110', '1100', '1101', '1001', '1011', '0011']

def get_PIDError(data):
    """
    计算并返回 error 值。
    ...
    reversed(right_L)
    try:
        index = L.index(left_L)
        error = index * -1
        print('left_L index:', index)
    except ValueError:
        pass

    try:
        index = L.index(right_L)
        error += index * 1
        print('right_L index:', index)
    except ValueError:
        pass

    print('data:', left_L, right_L)
