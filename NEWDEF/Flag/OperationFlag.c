...
uint8_t OFlag_alarmFlag = 0;    //烽火台回传数据
...
uint8_t LT_cmd[8] = {0x55, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0xBB};       //立体显示标志物
/*
******红外通讯********
1.智能路灯标志物
2.报警器标志物
3.立体显示器标志物（指令多参考通信协议） 自定义文本用的是Zigbee
4.LCD显示标志物
*****Zigbee通讯*******
1.LED显示标志物
2.道闸标志物
...
{
    if (data[0] == 0x55 && data[7] == 0xbb && (calc_CheckSum(data[2],data[3],data[4],data[5])==data[6])) //帧头 帧尾
    {
...
/**
 * @description: 烽火台获取随机救援坐标
 * @param {uint8_t} t 超时时间 单位：秒
 * @return {*}
 */
uint8_t OFlag_alarm_getP(uint8_t t)
{
    uint8_t _cmd[8] = {0x55, zNum_alarm, 0x09, 0x00, 0, 0, calc_CheckSum(zNum_alarm, 0x09, 0, 0), 0xbb};
    Agreement_Send(Agreement_mode_Zigbee, _cmd);
    if (OFlag_xx(_cmd,&OFlag_alarmFlag,t))
    {
        return OFlag_alarmFlag;
    }
    return 0;
}
...
/**
 * @description: 立体显示标志物自定义文本
 * @param {uint8_t} *str 数据字符串
 * @return {*}
 */
void OFlag_ltDisplay_Text(uint8_t *str)
{
    uint8_t dt[8];
    uint16_t p_len = strlen(str); // 文本长度
    int i;
    dt[0] = LT_cmd[0]; //帧头1
    dt[1] = LT_cmd[1]; //帧头2
...
unsigned int OFlag_DZ_wait(uint8_t count, uint8_t *str)
{
    uint8_t r = 1, t = 0;
    DZ_flag = 0;                                  //重置接收状态
    if (Zigbee_Rx_flag)Zigbee_Rx_flag=0;
    Send_Debug_string2("wait DZ");
    TIM_Cmd(TIM4, ENABLE);                      //开启接收
    OFlag_DZ_open(str); //发送多次请求回传
    OFlag_DZ(0x20,1,0,0);//请求回传道闸状态


    while (DZ_flag == 0) //等待回传结果
    {
        t++;
        if (t >= count) //超时跳出
        {
 ...
 * @param {uint8_t} mode 0.日期设置 1.时间设置 2.天气温度
 * @param {uint8_t} t1 年|时|天气     
 * @param {uint8_t} t2 月|分|温度      
 * @param {uint8_t} t3 日|秒
 * @return {*}
 * 例:日期2021年5月20日 OFlag_YY_setTime(0,0x21,0x05,0x20);
 */
void OFlag_YY_set(uint8_t mode, uint8_t t1, uint8_t t2, uint8_t t3)
{
    switch (mode)
    {
    case 0:
 ...
 * @param {uint8_t} mode 0.查询当前日期 1.查询当前时间 2.查询当前天气温度
 * @param {uint8_t} count 超时时长/s
 * @return {*}
 * 显示字符串形式转换例子
    char *time,buf[50];
    time= OFlag_YY_getTime(0,10);
    sprintf(buf,"%c%c:%c%c:%c%c",get_hexToChar(time[0])[0],get_hexToChar(time[0])[1],get_hexToChar(time[1])[0],get_hexToChar(time[1])[1],get_hexToChar(time[2])[0],get_hexToChar(time[2])[1]);
	Send_Debug_string2(buf);

    查询当前日期
    年月日 0x22 0x10 0x14 2022-10-14

    查询当前时间
    时分秒 0x10 0x27 0x47 10:27:47

    查询当前天气温度
    [0] = 0-5 大风、多云、晴、小雪、小雨、阴天
    [1] = 0x19 十六进制 25℃
    [2] = 0 
 */
uint8_t *OFlag_YY_get(uint8_t mode, uint32_t count)
{
    ...
    while ((0<count--) && (ZIGBEE_TIMRX_flag==0))        //超时判断
    {
        switch (mode)
        {
        case 0:
 ...
 * @param {uint8_t} *str
 * @param {uint8_t} encode 0.GB2312 1.GBK 2.BIG5 3.Unicode
 * @return {*}
 * |0xFD|0xXX |0xXX |·····|
 * |帧头|高字节|低字节|数据区|
 *         0     0     2     停止合成语音
 ...
 */
uint8_t OFlag_YY_play(uint8_t *str,uint8_t encode)
{   //别怀疑 测试很多次了没问题
    uint8_t data[100];
    ...
    data[3] = 0x01;                //命令字
    data[4] = encode;              //0.GB2312 1.GBK 2.BIG5 3.Unicode
...
/*
    ...
    data[0] = 0xFD;                //帧头
    data[1] = 0xff & ((p_len + 2) >> 8); // 数据区长度高八位
    data[2] = 0xff & (p_len + 2);        // 数据区长度低八位
    Send_ZigbeeData_To_Fifo(data, 5);
 ...
 * @return {*}
 */
uint8_t OFlag_CK_statusGet(uint8_t mode, uint8_t num, uint8_t count)
{
...
/**
 * @description: 主车过特殊地形 检测到白色线再次行驶n米
 * @param {unsigned int} speed 行驶速度
 * @param {unsigned int} refMP 参考码盘双检测版900/单检测1250 地形中间的未知片段 
 * @return {*} 
 * OFlag_DX_carGo(MasterCar_GoSpeed,1250);
 */
void OFlag_DX_carGo(unsigned int speed, unsigned int refMP)
{
    ...
    MasterCar_GoMP(MasterCar_GoSpeed, refMP); //通过地形
#if 0 //二次检测白线 

    _mp = Roadway_mp_Get();
    //防止没有找到第二条数据 这里跑出400就退出
    while (PID_Track4(speed - 6) != 99)
    {
        if(Roadway_mp_Get()>_mp+400)
        {
            break;
        }
        delay_ms(1);
    ...
    MasterCar_GoMP(MasterCar_GoSpeed, 100); //通过地形
#endif
...
uint8_t OFlag_SlaveRun_wait_flag(uint8_t count,uint8_t flag)
{
    ...
    OFlag_SLAVEflag = flag; //重置副车启动主车标志
    delay_ms(300);
    TaskBoard_Time(count);
 ...
 * @param {uint8_t} *cmd 请求回传协议包
 * @param {uint8_t} *flag 标志 初始自动重置0 结束标志为非0值
