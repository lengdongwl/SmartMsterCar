...
#define zNum_TFT_C 0x12     //TFTC显示标志物
...
#define zNum_JT_C 0x13      //智能交通标志物C
#define zNum_JT_D 0x14      //智能交通标志物D
...
void OFlag_WX(uint8_t cmd, uint8_t fcmd1, uint8_t fcmd2, uint8_t fcmd3);//无线充电标志物控制
void OFlag_WX_open(uint8_t *key); //无线充电标志物开启 10秒后自动关闭
void OFlag_WX_close(void);//无线充电标志物关闭
void OFlag_WX_change(uint8_t *key);//无线充电标志物修改key
...
uint8_t OFlag_ETC_runWait(uint32_t mp);                                   //ETC运行等待
...
void OFlag_SlaveRun2(uint8_t flag);                                                    //主车发送Zigbee指令启动副车
