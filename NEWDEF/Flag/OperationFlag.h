...
void OFlag_ltDisplay_show(uint8_t *CP, uint8_t *coordinate);                                   //车牌及坐标显示
...
void OFlag_YY_setTime(uint8_t mode, uint8_t t1, uint8_t t2, uint8_t t3); //设置起始日期或时间
uint8_t *OFlag_YY_getTime(uint8_t mode, uint32_t count);                 //读取起始日期或时间
uint8_t OFlag_YY_play(uint8_t *str);                                     //语音播报标志物合成指定语音文本
...
void OFlag_CK_statusGet(uint8_t mode, uint8_t num);                                    //立体车库标志物 请求返回车库状态信息
